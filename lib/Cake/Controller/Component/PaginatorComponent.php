...
class PaginatorComponent extends Component {
    ...
    public function paginate($object = null, $scope = array(), $whitelist = array()) {
        ...
        if ($object->hasMethod('paginateCount')) {
            ...
            //james 修改 在分页基础上增加4页作为最大页数，来提高效率 
            //todo 如果用户都封了10页了，就不管他了，暂时不会改  limit xx,xx 的模式
            if ($page > 10) {
                ...
                $limit2 = $page * $limit + 4 * $limit;
                $fields = array($fields[0]);
                $parameters = array('conditions' => $conditions, 'fields' => $fields, 'limit' => $limit2);
                if ($recursive != $object->recursive) {
                    $parameters['recursive'] = $recursive;
                }
                $results2 = $object->find($type, array_merge($parameters, $extra));
